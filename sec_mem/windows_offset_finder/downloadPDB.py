...
#    OSGuid: VM OS Guid
#    pdb_filename: Windows kernel symbols filename need to be downloaded
# return:
#    n_pdb_path: the path that Windows kernel symbols file from net download to
#===============================================================================
def downloadPDB_byNet(OSversion,OSGuid,pdb_filename):
    n_pdb_path = "/var/lib/node_ta/sec_mem/tmp/" + OSversion + ".pdb"
    #print "OSversion: %s" % OSversion
    #print "Guid: %s" % OSGuid
    #print "Name: %s" % pdb_filename
    #sys.exit(0)
    saved_file = download_file(OSGuid,pdb_filename,None)
    if saved_file is not None:
        if saved_file.endswith("_"):
            ...
            if retval is not 0:
                ...
                os.system("mv %s %s" % (saved_file[:-1], n_pdb_path))
            else:
                os.system("rm %s" % saved_file)
                savefile = saved_file[:-1] + 'b'
                os.system("mv %s %s" % (savefile, n_pdb_path))
    return "%s" % n_pdb_path
#OSversion = "Windows_7"
#OSGuid = "684da42a30cc450f81c535b4d18944b12"
#pdb_filename = "ntkrpamp.pdb"
#print downloadPDB_byNet(OSversion,OSGuid,pdb_filename)
